...
(I probably will be too lazy to do parts 2-4 in this. I am moving to a script, which is what I use for the videos anyway.)
...
**The Counterfactual:**
The regression predictions are shown with the solid lines, and the counterfactual for the treatment group is shown with the red dotted line. What is a counterfactual? It is what we think would have happened without treatment, so we can guess the effect of treatment. In this case, it is the regression output but with the interaction term set to 0. Why does this work? Because the trends are parallel! The dotted line shows what would theoretically happen in `r tgroup` if they followed the trend but did not have STEM, where the solid red line shows what actually happened with STEM and the time trend. The gap between the solid red line and the dotted red line is the treatment effect. It shows how much the gap closed between red and blue.

As a side note, both models give very similar results. The treatment effect in the first model with the discrete time trend is `r round(dd_reg1$coefficients[2],2)`, while the treatment effect with the linear time trend is `r round(dd_reg2$coefficients[2],2)`. It won't always be so clean. This is fake data.
...
**Takeaway:** Difference in Differences relies on a 'natural experiment,' wherein the real world separates a treatment and control group and doses treatment. If trends for treatment and control groups are parallel, we can use DiD to exploit the natural experiment to estimate the causal impact of treatment to the average person in the treatment group. This involves a regression with at least 3 specific variables, and it will likely also need special standard errors. 
